...
public class TreeMolecule implements Molecule {
    ...
    public boolean contains(Atom target) {        
        return searchExistenceOfAtom(first,target)==null?false:true;//if result is null then the tree does not contain atom a
    }
        ...
        if (thisAtom.getBonds().size() == 0)//if have no children
            ...
            for (Bond b : thisAtom.getBonds()) {
                ...
                if (thisAtom.getBonds().size() > 1)
                    s += "(" + bondValue + createSubSmileString(b.getChild()) + ")";// if this atom only has more than
