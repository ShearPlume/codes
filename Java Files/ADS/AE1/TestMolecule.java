...
public class TestMolecule {
	...
	public static ArrayList<Atom> atomList(int len, String type) {
		...
		for (int i = 0; i < len; i++)
			theAtoms.add(new Atom(type));// supply of atoms of the specified type
		return theAtoms;

	}

	// these are so that I have some stored, nothing to do with the actual
	// implementation
	public static TreeMolecule createMolecule(String name, ArrayList<Atom> carbonAtoms, ArrayList<Atom> oxygenAtoms,
			ArrayList<Atom> nitrogenAtoms) {

		TreeMolecule mol = new TreeMolecule(carbonAtoms.get(0));// all molecules contain an initial carbon atom
		...
		if (name.equals("5-amino-4-methylpentanamide")) {// 5-amino-4-methylpentanamide
			...
			mol.addBond(carbonAtoms.get(5), nitrogenAtoms.get(0), 1);
			return mol;
		}
